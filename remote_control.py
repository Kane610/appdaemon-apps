...
class RemoteControlBase(hass.Hass):
    ...
    def initialize(self):
        ...
        for lights in self.click.values():
            click_lights = {**click_lights, **light_master.get(entity_ids=lights)}

        hold_lights = {}
        for lights in self.hold.values():
            hold_lights = {**hold_lights, **light_master.get(entity_ids=lights)}

        self.lights = {**click_lights, **hold_lights}
        ...
        # self.set_brightness_all_lights(150)
    ...
    def turn(self, action, device_dict):
        ...
        if button_counter not in device_dict:
            return
    ...
    def set_brightness(self, light, dim):
        ...
        """
        brightness = light.attributes.get("brightness", 122) + dim
    ...
    def controlled_device(self):
        ...
        light = list(self.lights.keys())[self.controlled_device_index]
    ...
    def select_device(self, device_index, flash=True):
        ...
        if flash and self.controlled_device.entity_id.startswith("light."):
            self.controlled_device.flash()
...
class HueDimmer(RemoteControlBase):
    ...
    def handle_button_event(self):
        """"""
        if self.button_event == self.POWER_ON + BUTTON_PRESS:
            self.log("POWER ON PRESSED")
        elif self.button_event == self.POWER_ON + BUTTON_RELEASE:
            self.log("POWER ON RELEASED, check multi click")
            self.turn_click("on")
        elif self.button_event == self.POWER_ON + BUTTON_LONG_PRESS:
            self.log("POWER ON LONG PRESS, check long press")
            self.turn_hold("on")
        elif self.button_event == self.POWER_ON + BUTTON_LONG_RELEASE:
            self.log("POWER ON LONG RELEASE")

        elif self.button_event == self.DIM_UP + BUTTON_PRESS:
            self.log("DIM UP PRESSED")
        elif self.button_event == self.DIM_UP + BUTTON_RELEASE:
            self.log("DIM UP RELEASED, check multi click")
            self.set_brightness_all_lights(+40)
        elif self.button_event == self.DIM_UP + BUTTON_LONG_PRESS:
            self.log("DIM UP LONG PRESS, check long press")
            self.set_brightness_all_lights(+40)
        elif self.button_event == self.DIM_UP + BUTTON_LONG_RELEASE:
            self.log("DIM UP LONG RELEASE")

        elif self.button_event == self.DIM_DOWN + BUTTON_PRESS:
            self.log("DIM DOWN PRESSED")
        elif self.button_event == self.DIM_DOWN + BUTTON_RELEASE:
            self.log("DIM DOWN RELEASED, check multi click")
            self.set_brightness_all_lights(-40)
        elif self.button_event == self.DIM_DOWN + BUTTON_LONG_PRESS:
            self.log("DIM DOWN LONG PRESS, check long press")
            self.set_brightness_all_lights(-40)
        elif self.button_event == self.DIM_DOWN + BUTTON_LONG_RELEASE:
            self.log("DIM DOWN LONG RELEASE")

        elif self.button_event == self.POWER_OFF + BUTTON_PRESS:
            self.log("POWER OFF PRESSED")
        elif self.button_event == self.POWER_OFF + BUTTON_RELEASE:
            self.log("POWER OFF, check multi click")
            self.turn_click("off")
        elif self.button_event == self.POWER_OFF + BUTTON_LONG_PRESS:
            self.log("POWER OFF PRESS, check long press")
            self.turn_hold("off")
        elif self.button_event == self.POWER_OFF + BUTTON_LONG_RELEASE:
